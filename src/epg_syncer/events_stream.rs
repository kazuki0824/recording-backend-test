...
impl ProgramsIndexManager {
    pub(crate) async fn update_db_from_stream(
        &self,
    ) -> impl Iterator<Item = Result<Service, Box<dyn Error>>> {
        // subscribe_to_events_api
        get_events_stream(&self.m_conf, EventResource::Service, None)
            .unwrap()
            .map(|value| match value {
                Ok(value) if value.resource == EventResource::Service => {
                    from_value::<Service>(value.data).map_err(|e| Box::new(e) as Box<dyn Error>)
                }
                Ok(_) => Err(Box::new(DeError {
                    reason: DeErrReason::ResourceKindMismatch,
                }) as Box<dyn Error>),
