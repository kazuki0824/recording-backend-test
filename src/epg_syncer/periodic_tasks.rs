...
use crate::mirakurun_client::{fetch_programmes, fetch_services, ProgramsReturnType, ServicesReturnType};
...
impl EpgSyncManager {
    ...
    pub(crate) async fn refresh_db(&self) -> Result<(), Error> {
        ...
        info!("{:?}", push_programs_ranges(&self.index_programs, &initial_epg.1.unwrap()).await?);
        info!("{:?}", push_services_ranges(&self.index_programs, &initial_epg.0.unwrap()).await?);
        Ok(())
    }
}
